...
//  Vector4u* rgb =
//      viewHierarchy->levels[levelId]->rgb->GetData(MEMORYDEVICE_CPU);
//
//  final_f = 0;
//  countedPoints_valid = 0;
//  for (int locId = 0; locId < noTotalPoints; locId++) {
//    float colorDiffSq = getColorDifferenceSq(locations, colours, rgb, imgSize,
//                                             locId, projParams, M);
//    if (colorDiffSq >= 0) {
//      final_f += colorDiffSq;
//      countedPoints_valid++;
...
//      viewHierarchy->levels[levelId]->rgb->GetData(MEMORYDEVICE_CPU);
//  Vector4s* gx =
//      viewHierarchy->levels[levelId]->gradientX_rgb->GetData(MEMORYDEVICE_CPU);
//  Vector4s* gy =
//      viewHierarchy->levels[levelId]->gradientY_rgb->GetData(MEMORYDEVICE_CPU);
//
//  for (int locId = 0; locId < noTotalPoints; locId++) {
//    float localGradient[6], localHessian[21];
//
//    bool isValidPoint = computePerPointGH_rt_Color(
//        localGradient, localHessian, locations, colours, rgb, imgSize, locId,
