...
__global__ void externalTrackerOneLevel_f_device(
  ...
  if (locId_global < noTotalPoints) {
    float externalDiffSq = getExternalDifferenceSq(
        locations, colours, rgb, imgSize, locId_global, projParams, M);
    if (externalDiffSq >= 0) {
      out_shared[locId_local].x = externalDiffSq;
...
__global__ void externalTrackerOneLevel_g_rt_device(
  ...
  if (locId_global < noTotalPoints) {
    computePerPointGH_rt_External(localGradient, localHessian, locations,
                                  colours, rgb, imgSize, locId_global,
  ...
  for (int paraId = 0; paraId < numPara; paraId++) {
    ...
    if (threadIdx.x == 0)
      g_out[blockIdx.x * numPara + paraId] = dim_shared[locId_local];
  }
  ...
  for (int paraId = 0; paraId < numParaSQ; paraId++) {
    ...
    if (threadIdx.x == 0)
      h_out[blockIdx.x * numParaSQ + paraId] = dim_shared[locId_local];
  }
}
