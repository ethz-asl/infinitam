...
class ITMTrackerFactory {
 ...
 private:
  typedef ITMTracker *(*Maker)(const Vector2i&, const ITMLibSettings*,
                               const ITMLowLevelEngine*, ITMIMUCalibrator*,
 ...
 private:
  ...
  ITMTrackerFactory() {
    ...
    makers.insert(
        std::make_pair(ITMLibSettings::TRACKER_EXTERNAL, &MakeExternalTracker));
    makers.insert(
        std::make_pair(ITMLibSettings::TRACKER_WICP, &MakeWeightedICPTracker));
 ...
 public:
   ...
   */
  ITMTracker *Make(const Vector2i& trackedImageSize,
                   const ITMLibSettings *settings,
                   const ITMLowLevelEngine *lowLevelEngine,
                   ITMIMUCalibrator *imuCalibrator,
                   ITMScene<TVoxel, TIndex> *scene) const {
    typename std::map<ITMLibSettings::TrackerType, Maker>::const_iterator it =
        makers.find(settings->trackerType);
    if (it == makers.end())
 ...
 public:
  ...
  static ITMTracker *MakeExternalTracker(
    ...
    switch (settings->deviceType) {
      case ITMLibSettings::DEVICE_CPU: {
        return new ITMExternalTracker_CPU(trackedImageSize,
                                          settings->trackingRegime,
                                          settings->noHierarchyLevels,
                                          lowLevelEngine);
      }
    ...
    switch (settings->deviceType) {
      case ITMLibSettings::DEVICE_CPU: {
        return new ITMDepthTracker_CPU(
            trackedImageSize, settings->trackingRegime,
            settings->noHierarchyLevels, settings->noICPRunTillLevel,
            settings->depthTrackerICPThreshold,
            settings->depthTrackerTerminationThreshold, lowLevelEngine);
      }
      case ITMLibSettings::DEVICE_CUDA: {
#ifndef COMPILE_WITHOUT_CUDA
   ...
   */
  static ITMTracker *MakeWeightedICPTracker(
      const Vector2i& trackedImageSize, const ITMLibSettings *settings,
      const ITMLowLevelEngine *lowLevelEngine, ITMIMUCalibrator *imuCalibrator,
      ITMScene<TVoxel, TIndex> *scene) {
    switch (settings->deviceType) {
    ...
    switch (settings->deviceType) {
      case ITMLibSettings::DEVICE_CPU: {
        ...
        compositeTracker->SetTracker(
            new ITMDepthTracker_CPU(trackedImageSize, settings->trackingRegime,
                                    settings->noHierarchyLevels,
                                    settings->noICPRunTillLevel,
                                    settings->depthTrackerICPThreshold,
                                    settings->depthTrackerTerminationThreshold,
  ...
  static ITMTracker *MakeRenTracker(const Vector2i& trackedImageSize,
    ...
    switch (settings->deviceType) {
      case ITMLibSettings::DEVICE_CPU: {
        return new ITMRenTracker_CPU<TVoxel, TIndex>(trackedImageSize,
                                                     settings->trackingRegime,
                                                     2, lowLevelEngine, scene);
      }
      case ITMLibSettings::DEVICE_CUDA: {
#ifndef COMPILE_WITHOUT_CUDA
