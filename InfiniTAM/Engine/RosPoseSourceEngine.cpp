...
void RosPoseSourceEngine::TFCallback(const tf::tfMessage& tf_msg) {
  ...
  //    }
  //
  //    // Get the pose of the camera from the same time stamp as the latest
  //    image.
  ...
  //
  //    // Invert the rotation since we use this in the perspective of the
  //    camera.
...
void RosPoseSourceEngine::getMeasurement(ITMPoseMeasurement* pose) {
  std::cout << "getMeasurement(ITMPoseMeasurement* pose)" << std::endl;
  if (cached_pose != NULL) {
