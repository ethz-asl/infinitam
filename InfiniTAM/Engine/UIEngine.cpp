...
void UIEngine::glutDisplayFunction() {
  ...
  if (uiEngine->freeviewActive) {
    sprintf(str, "n - next frame \t b - all frames \t e/esc - exit \t f - "
    ...
    sprintf(str, "n - next frame \t b - all frames \t e/esc - exit \t f - free "
...
void UIEngine::glutIdleFunction() {
  ...
  //		glReadPixels(0, 0, saveImage->noDims.x, saveImage->noDims.x,
  // GL_RGBA, GL_UNSIGNED_BYTE, (unsigned char*)saveImage->GetData(false));
  //		sprintf(outFile, "%s/out_%05d.ppm", uiEngine->outFolder,
  // uiEngine->processedFrameNo);
...
void UIEngine::glutMouseMoveFunction(int x, int y) {
  ...
  case 2: {
    ...
    uiEngine->freeviewPose.SetT(uiEngine->freeviewPose.GetT() +
                                scale_translation * Vector3f((float)movement.x,
                                                             (float)movement.y,
                                                             0.0f));
    uiEngine->needsRefresh = true;
    break;
