...
RealSenseEngine::RealSenseEngine(const char *calibFilename,
  ...
  data->dev->enable_stream(rs::stream::depth, imageSize_d.x, imageSize_d.y,
                           rs::format::z16, 60);
  data->dev->enable_stream(rs::stream::color, imageSize_rgb.x, imageSize_rgb.y,
                           rs::format::rgb8, 60);

  rs::intrinsics intrinsics_depth =
      data->dev->get_stream_intrinsics(rs::stream::depth);
  // printf("intrinsics_depth: %f, %f, %f, %f", intrinsics_depth.fx,
  //        intrinsics_depth.fy, intrinsics_depth.ppx, intrinsics_depth.ppy);
  rs::intrinsics intrinsics_rgb =
      data->dev->get_stream_intrinsics(rs::stream::color);

  this->calib.intrinsics_d.SetFrom(
      intrinsics_depth.fx, intrinsics_depth.fy, intrinsics_depth.ppx,
      intrinsics_depth.ppy, requested_imageSize_d.x, requested_imageSize_d.y);
  this->calib.intrinsics_rgb.SetFrom(
      intrinsics_rgb.fx, intrinsics_rgb.fy, intrinsics_rgb.ppx,
      intrinsics_rgb.ppy, requested_imageSize_rgb.x, requested_imageSize_rgb.y);
  this->calib.disparityCalib.params =
      Vector2f(data->dev->get_depth_scale(), 0.0f);
...
Vector2i RealSenseEngine::getDepthImageSize(void) { return Vector2i(0, 0); }
Vector2i RealSenseEngine::getRGBImageSize(void) { return Vector2i(0, 0); }
